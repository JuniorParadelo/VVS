...
public class BetServiceTest {
	...
	private UserProfile createUser() throws DuplicateInstanceException {
		return userService.registerUser("user", "password", 
				new UserProfileDetails("name", "lname", "name@email.es"));
	}
	...
	@Test
	public void testCreateBetAndFindBet() throws InstanceNotFoundException, DuplicateInstanceException, EventEmpezadoException, PastEventException, DuplicateEventException, BetTypesIgualesException {
    ...
    @Test
    public void testFindBetsByUserId() throws InstanceNotFoundException, DuplicateInstanceException, EventEmpezadoException, PastEventException, DuplicateEventException, BetTypesIgualesException{
       ...
       for (int i = 0; i < numberOfBets; i++) {
        	expectedBets.add(betService.createBet(betType.getOptions().get(0).getId(),
        			user.getUserProfileId(),(float) 15.00));
        }
       ...
       UserProfile user2 = userService.registerUser("user2", "pword", 
        ...
        betService.createBet(betType.getOptions().get(2).getId(),
        		user2.getUserProfileId(), (float) 15.00);
        ...
        short resultIndex = 0;
        do {
            betBlock = betService.findBetsByUserId(user.getUserProfileId(),
                    startIndex, count);
